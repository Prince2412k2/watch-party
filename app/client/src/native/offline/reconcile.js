...
export function reconcileList(map, list) {
  const next = new Map()
...
export function applyStart(map, { id, itemId, title, parts }) {
...
export function applyProgress(map, payload) {
  const next = new Map(map)
...
export function applyDone(map, payload) {
  const next = new Map(map)
  const prev = next.get(payload.id)
  next.set(payload.id, {
    ...prev,
...
export function applyError(map, payload) {
  const next = new Map(map)
  const prev = next.get(payload.id)
...
export function toSortedList(map) {

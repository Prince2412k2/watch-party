...
import { __setMockTransport, invoke, listen } from '../ipc.ts'
import { IPC, EVENTS } from '../contract.ts'
import { reconcileList, applyStart, applyProgress, applyDone, applyError, toSortedList } from './reconcile.ts'
import type { DownloadRecord } from '../contract.ts'
import { isDownloadDone, isDownloadError, isDownloadProgress, isDownloadRecord } from '../guards.ts'
import type { DownloadDonePayload, DownloadErrorPayload, DownloadProgressPayload } from './reconcile.ts'
...
function makeMockTransport() {
  type EventPayload = DownloadProgressPayload | DownloadDonePayload | DownloadErrorPayload
  type EventHandler = (event: { payload: EventPayload }) => void
  const state: { records: DownloadRecord[] } = { records: [] }
  const handlers: Partial<Record<string, EventHandler>> = {}
  const invokeCalls: Array<[string, unknown]> = []

  const mockInvoke = async (cmd: string, payload?: unknown): Promise<unknown> => {
    invokeCalls.push([cmd, payload])
    if (cmd === IPC.DL_LIST) return state.records
    if (cmd === IPC.DL_START) return { id: 'dl-1' }
    return {}
  }
  const mockListen = async (eventName: string, handler: (event: { payload: unknown }) => void): Promise<() => void> => {
    const typedHandler: EventHandler = (event) => handler(event)
    handlers[eventName] = typedHandler
    return () => {
      delete handlers[eventName]
    }
  }
  const fire = (eventName: string, payload: EventPayload): void => handlers[eventName]?.({ payload })
...
test('reconcile core: queued -> active -> done via dl:progress/dl:done, seeded from dl_list()', async () => {
  ...
  const initial = await invoke(IPC.DL_LIST, {})
  let map = reconcileList(new Map(), Array.isArray(initial) ? initial.filter(isDownloadRecord) : [])
  ...
  await listen(EVENTS.DL_PROGRESS, ({ payload }) => {
    if (isDownloadProgress(payload)) map = applyProgress(map, payload)
  })
  await listen(EVENTS.DL_DONE, ({ payload }) => {
    if (isDownloadDone(payload)) map = applyDone(map, payload)
  })
  await listen(EVENTS.DL_ERROR, ({ payload }) => {
    if (isDownloadError(payload)) map = applyError(map, payload)
  })
  ...
  const refreshed = await invoke(IPC.DL_LIST, {})
  map = reconcileList(map, Array.isArray(refreshed) ? refreshed.filter(isDownloadRecord) : [])
  assert.equal(map.get('dl-1')?.state, 'done')

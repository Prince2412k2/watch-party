> **ARCHIVED — HISTORICAL. DO NOT BRING UP THIS STACK.** The servarr services
> are defined in the root `docker-compose.yml`, which has owned them since it
> became the single source of truth — the live containers on the production host
> are all labelled with it. The two files this guide tells you to use,
> `docker-compose.servarr.yml` and `.env.servarr.example`, were removed in #63:
> running them alongside `docker-compose.yml` would have started a second copy
> of every service over the same config and media volumes.
>
> Bring the stack up with `docker compose up -d` from the repo root instead.
> Kept for the parts that are still true and are not written down elsewhere:
> the "why one /data mount" reasoning, and the Prowlarr/Sonarr/Radarr/Bazarr
> first-run wiring.

# Media-acquisition stack — bring-up guide (Phase 5.0, ARCHIVED)

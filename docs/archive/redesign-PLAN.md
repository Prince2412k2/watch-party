> **ARCHIVED — HISTORICAL.** The cinematic-minimal redesign this plan directs
> has shipped; the agent cards below were all executed. The authoritative design
> reference is now `docs/watchparty-design/README.md`, which supersedes the
> tokens and rules here (notably: the UI is no longer dark-only — three
> persisted themes are mandated). Kept as provenance for the direction.

# Watchparty Redesign — Execution Plan (ARCHIVED)

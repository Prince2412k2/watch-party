> **ARCHIVED — HISTORICAL. DO NOT BUILD OR OPERATE FROM THIS DOCUMENT.**
>
> This was the original build brief. The system diverged from it long ago, and
> in at least three places following it now would be actively wrong:
>
> - **"Do not build a custom sync engine. Use Jellyfin's SyncPlay API."**
>   Commit `84e5885` replaced SyncPlay with a host-authority timeline engine.
>   The real design is `app/server/session.js` (the schedule),
>   `app/client/src/sync/syncCore.ts` and `flutter_app/lib/sync/sync_core.dart`
>   (the clients), with the wire contract pinned in `app/shared/contracts/`.
> - **"Mobile is out of scope."** There is now a Flutter desktop/mobile client
>   (`flutter_app/`) and a PWA. See `docs/flutter/PLAN.md`.
> - **"LiveKit needs UDP 50000-50020 open."** Production runs single-port mux
>   on **7882** (`rtc.udp_port` in the LiveKit config). An operator writing
>   firewall rules from this document would open 21 ports that do nothing and
>   still not open the one that matters. See
>   `docs/specs/2026-08-05-i60-deploy-hardening.md` §FR-002 and `deploy/firewall.sh`.
>
> Kept only as provenance for why the project started where it did.
> `docs/specs/2026-08-05-i60-deploy-hardening.md` cites it by line number.

# Watchparty App — Claude Code Handoff Document (ARCHIVED)

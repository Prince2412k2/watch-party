> **ARCHIVED — HISTORICAL.** This is the read-only audit that produced issues
> #54–#64. It describes the codebase as it stood before any of them were fixed,
> so most of its findings are no longer true — the committed `devkey`/`changeme`
> secrets, the failing suites, the unauthenticated proxies. Read the issues and
> `docs/specs/` for what was actually done. Kept as provenance for why that work
> exists.

# Watch-Party Codebase Audit (ARCHIVED)
